...
job_seeker_name = st.sidebar.text_input("参加者の名前を入力してください", placeholder="例:山田花子")
...
if uploaded_file is not None:
    ...
    prompt1 = f"""
あなたは、{interviewee_name}と{job_seeker_name}の商談音声から正確で詳細な議事録を作成する専門家です。以下の指示に従って、高品質な議事録を作成してください。
{interviewee_name}は人材紹介サービスを運営しています。{job_seeker_name}に自社の人材紹介サービスを利用してもらいたいです。
その為に、{interviewee_name}は{job_seeker_name}に現在の人材状況のヒアリングをおこなっており、あなたには、そのヒアリング内容について、まとめてほしいです。
...
1. 商談背景
   - {job_seeker_name}はなぜ、今回の商談に参加しようと思ったのか。

2. 採用背景
   - {job_seeker_name}の採用に関する現在の状況。

3. 採用計画
   - {job_seeker_name}の採用をどのように行っていきたいか。

4. 採用計画に対する進捗状況
   - {job_seeker_name}の採用計画の進捗状況。

5. 採用に伴う課題
   - {job_seeker_name}の現在採用に対して、抱えている課題。

6. 今までの採用手法
   - {job_seeker_name}の今までの採用手法

7. 現在利用している他社サービス
   - {job_seeker_name}の現在利用している他社サービス

8. 他社サービスを利用しての所感
   - {job_seeker_name}の現在利用している他社サービスを利用しての所感

9. 給与
   - {job_seeker_name}が求めている人材の給与水準

10. 年齢
    - {job_seeker_name}が求めている人材の年齢

11. 転職回数
    - {job_seeker_name}が求めている人材の転職回数

12. 性別
    - {job_seeker_name}が求めている人材の性別

13. 学歴
    - {job_seeker_name}が求めている人材の学歴

14. 資格
    - {job_seeker_name}が求めている人材の資格情報

15. その他
    - {job_seeker_name}が求めている人材のその他の情報

16. 性格
    - {job_seeker_name}が求めている人材の性格

17. 持ち合わせる経験
    - {job_seeker_name}が求めている人材の持ち合わせる経験

18. 活躍する人の傾向
    - {job_seeker_name}の企業で活躍する人材の傾向の情報

19. 事業内容
    - {job_seeker_name}の企業の事業内容

20. 具体的な業務
    - {job_seeker_name}の企業の具体的な業務内容

21. １日の流れ
    - {job_seeker_name}の企業の具体的な１日の流れ

22. 企業設立背景
    - {job_seeker_name}の企業の設立背景

23. 企業の雰囲気・文化
    - {job_seeker_name}の企業の雰囲気・文化

24. 他社との違い
    - {job_seeker_name}の企業の他社との違い

25. 選考回数
    - {job_seeker_name}の設定している選考回数

26. 選考の流れ
    - {job_seeker_name}の設定している選考の流れ

27. 手数料
    - {job_seeker_name}と{interviewee_name}が商談を行い、最終的に両社が合意した手数料

28. 返金規定
    - {job_seeker_name}と{interviewee_name}が商談を行い、最終的に両社が合意した返金規定

29. 次回アクション
    - {job_seeker_name}と{interviewee_name}が次回行うべきアクションを箇条書き

30. その他ユニークポイント
    - ここまでの項目に存在していない{job_seeker_name}の企業の興味深い点やアピールポイントを記載
